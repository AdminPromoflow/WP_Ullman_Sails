...
(() => {
  ...
  const revealSection = (section) => {
    ...
    // Delays: from data-sr-delay, else stagger fallback
    items.forEach((el, i) => {
    ...
    // Reduced motion: show instantly (still one-time)
    if (prefersReduced) {
      items.forEach((el) => el.classList.add("is-revealed"));
      return;
    }

    // One-time reveal (uses per-item delay)
    items.forEach((el) => {
